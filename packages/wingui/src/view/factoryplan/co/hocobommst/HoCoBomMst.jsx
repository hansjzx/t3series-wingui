...
function HoCoBomMst(props) {
  ...
  function loadDataGrid1() {
    ...
    zAxios({
      ...
      url: baseURI() + 'fp/co/HO_CO_BOM_MST/q1',
      data: params
    })
    .then(function (res) {
  ...
  return (
    <>
      <ContentInner>
        ...
        <ButtonArea title={transLangKey("HO_CO_BOM_MST")}>
          <LeftButtonArea>
          </LeftButtonArea>
          <RightButtonArea>
          </RightButtonArea>
        ...
        <ResultArea sizes={[100, 50]} direction={"vertical"}>
          <BaseGrid id="grid1" items={grid1Items} viewCd="HO_CO_BOM_MST" gridCd="HO_CO_BOM_MST-RST_CPT_01" userName={username} afterGridCreate={afterGridCreate}></BaseGrid>
        </ResultArea>
        <StatusArea message={message} show={false}>
          <GridCnt grid="grid1" format={"{0} " + transLangKey("CASES") + " " + transLangKey("MSG_0010")}></GridCnt>
        </StatusArea>
      </ContentInner>
      {itemPopupOpen && <PopSelectItem open={itemPopupOpen} onClose={() => setItemPopupOpen(false)} confirm={setItemCd} viewId={"HO_CO_BOM_MST"}></PopSelectItem>}
      {plntPopupOpen && <PopSelectPlnt open={plntPopupOpen} onClose={() => setPlntPopupOpen(false)} confirm={setPlntCd} viewId={"HO_CO_BOM_MST"}></PopSelectPlnt>}
      <PopPersonalize open={personalizeOpen} onClose={() => setPersonalizeOpen(false)} resetCallback={reloadPrefInfo} viewCd="HO_CO_BOM_MST" grid={grid1} username={username} authTpId={""}></PopPersonalize>
    </>
  );
}

export default HoCoBomMst;
