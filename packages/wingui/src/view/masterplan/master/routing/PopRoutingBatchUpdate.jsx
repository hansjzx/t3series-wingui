...
function PopRoutingBatchUpdate(props) {
  ...
  function batchUpdate() {
    ...
    zAxios({
      method: 'post',
      url: baseURI() + 'engine/mp/SRV_UI_MP_38_BATCH',
      data: formData,
      fromPopup: true
    })
    .then(function (res) {
      if (res.data.RESULT_SUCCESS) {
        showMessage(transLangKey('MSG_CONFIRM'), transLangKey(res.data.RESULT_DATA.IM_DATA.SP_UI_MP_38_BATCH_P_RT_MSG), { close: false });
      } else {
  ...
  return (
    <PopupDialog open={props.open} onClose={props.onClose} onSubmit={batchUpdate} title='공정 관리 일괄반영' resizeWidth={250} resizeHeight={180}>
      <InputField type='check' name='overwriteDataYn' control={control} options={[{ label: transLangKey('OVERWRITE_EXIST_DATA'), value: 'Y' }]} />
    </PopupDialog>
  )
}

export default PopRoutingBatchUpdate;
