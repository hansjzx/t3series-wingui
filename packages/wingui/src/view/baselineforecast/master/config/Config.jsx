...
import { ContentInner, ResultArea, SearchArea, StatusArea, ButtonArea, LeftButtonArea, RightButtonArea, SearchRow, InputField, GridAddRowButton, GridDeleteRowButton, BaseGrid, GridCnt, useViewStore, useUserStore, GridSaveButton, zAxios } from "@zionex/wingui-core/src/common/imports";
...
function Config() {
  ...
  const getNewGridData1 = () => {
    ...
    return {
      ...
      MODULE_NM: moduleValue.filter((row) => row.value === currentModuleValue)[0].label,
    };
  };
  ...
  const onDelete = (targetGrid, deleteRows) => {
    ...
    if (deleteRows.length > 0) {
      return zAxios({
        method: "post",
        url: baseURI() + "engine/dp/SRV_SET_SP_UI_DP_01_" + (targetGrid.gridId === "grid1" ? "D1" : "D2"),
  ...
  return (
    <ContentInner>
      <SearchArea>
        <SearchRow>
          <InputField name="moduleValue" label={transLangKey("MODULE_VAL")} type="select" control={control} options={[{ label: transLangKey("BF"), value: "BF" }]} />
        </SearchRow>
      </SearchArea>
      <ResultArea>
        <Box>
          <ButtonArea title={transLangKey("GRP_CONFIG")}>
            <RightButtonArea>
              <GridDeleteRowButton grid="grid1" type="icon" onDelete={onDelete} onAfterDelete={(targetGrid) => loadGridData(targetGrid.gridId, getValues("moduleValue"))} />
        ...
        <Box>
          <ButtonArea title={transLangKey("UI_BF_01")}>
            <LeftButtonArea></LeftButtonArea>
            <RightButtonArea>
              <ButtonGroup>
                <GridAddRowButton grid="grid2" type="icon" onBeforeAdd={onBeforeAdd} onGetData={getNewGridData2} />
                <GridDeleteRowButton grid="grid2" type="icon" onDelete={onDelete} onAfterDelete={(targetGrid) => loadGridData(targetGrid.gridId, getValues("confGrpId"))} />
