...
function PopForecastResultItem(props) {
  ...
  useEffect(() => {
    ...
    if (grdObjPopup) {
      if (grdObjPopup.dataProvider) {
        if (itemGrid != grdObjPopup) setItemGrid(grdObjPopup);
      }
    }
  }, [viewData]);
  ...
  const loadItemLvComboListFromEngine = () => {
    ...
    zAxios({
      method: "post",
      url: "engine/dp/SRV_GET_SP_UI_DP_00_USER_ITEM_LV_Q1",
      params: param,
    })
      .then(function (res) {
        let array = [];
  ...
  return (
    <PopupDialog open={props.open} onClose={props.onClose} title={props.title} onSubmit={handleSubmit(saveSubmit, onError)} resizeHeight={400} resizeWidth={960}>
      <SearchArea displaySize="small" submit={handleSubmit(onPopupSubmit, onError)} expandButton={false} searchButton={true}>
        ...
        <InputField type={"select"} name={"itemLvCd"} label={transLangKey("ITEM_LV_CD")} control={control} options={itemsTpOptions} readonly={false} disabled={false} displaySize="small" />
        {/* <IconButton onClick={()=> {onPopupSubmit() }}><Icon.Search/></IconButton> */}
      </SearchArea>
      <Box style={{ height: "100%" }}>
        <BaseGrid id={`${props.id}_ForecastResultItemGrid`} items={popupGrid1Items}></BaseGrid>
      </Box>
    </PopupDialog>
  );
}
