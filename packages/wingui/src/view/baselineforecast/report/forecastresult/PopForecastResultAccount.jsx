...
import { ContentInner, ViewPath, ResultArea, SearchArea, StatusArea, ButtonArea, LeftButtonArea, RightButtonArea, SearchRow, SplitPanel, InputField, GridAddRowButton, GridDelRowButton, CommonBtn, BaseGrid, GridCnt, useViewStore, useStyles, zAxios } from "@zionex/wingui-core/src/common/imports";
...
function PopForecastResultAccount(props) {
  ...
  useEffect(() => {
    ...
    if (grdObjPopup) {
      if (grdObjPopup.dataProvider) {
        if (accountGrid != grdObjPopup) setAccountGrid(grdObjPopup);
      }
    }
  }, [viewData]);
  ...
  const loadItemLvComboListFromEngine = () => {
    ...
    zAxios({
      method: "post",
      url: "engine/dp/SRV_GET_SP_UI_DP_00_USER_SALES_LV_Q1",
      params: param,
    })
      .then(function (res) {
        let array = [];
  ...
  return (
    <PopupDialog open={props.open} onClose={props.onClose} onSubmit={handleSubmit(saveSubmit, onError)} title={props.title} resizeHeight={400} resizeWidth={1150}>
      <SearchArea displaySize="small" submit={handleSubmit(onPopupSubmit, onError)} expandButton={false} searchButton={true}>
        <InputField name="accountCd" label={transLangKey("ACCOUNT_CD")} readonly={false} disabled={false} control={control} displaySize="small" />
        <InputField name="accountNm" label={transLangKey("ACCOUNT_NM")} control={control} readonly={false} disabled={false} displaySize="small" />
        <InputField type={"select"} name={"salesLvCd"} label={transLangKey("SALES_LV_CD")} control={control} options={itemsTpOptions} readonly={false} disabled={false} displaySize="small" />
        {/* <IconButton onClick={()=> {onPopupSubmit() }}><Icon.Search/></IconButton> */}
      </SearchArea>
      <Box style={{ height: "100%" }}>
        <BaseGrid id={`${props.id}_ForecastResultAccountGrid`} items={popupGrid1Items}></BaseGrid>
      </Box>
    </PopupDialog>
  );
}
