...
import { SearchArea, ButtonArea, RightButtonArea, InputField, BaseGrid, useViewStore, zAxios } from "@zionex/wingui-core/src/common/imports";
import { showMessage } from "@wingui";
...
function PopPersonalize(props) {
  ...
  useEffect(() => {
    if (getValues("userGrp") !== undefined) {
      loadData(getValues());
      onGridGroupComboboxChanged(props.viewCd);
    }
  }, [watch("userGrp")]);
  ...
  useEffect(() => {
    if (gridNm && props.open) {
      onGridComboboxChanged();
      loadPreferenceDetailGroups(gridNm, props.username);
    }
  }, [gridNm, props.username, props.open]);

  useEffect(() => {
    if (userGroupOptions && userGroupOptions.length > 0) {
      if (props.userGrp && props.userGrp !== "") {
        let baseGrpCd;
        let psnl = localStorage.getItem("preference-group");
//        console.log("psnl", psnl);
        let saveGrpCd = "";
        if (psnl && psnl !== "" && psnl !== undefined && psnl.length > 0) {
          saveGrpCd = JSON.parse(psnl).filter(function (row) {
            return row.viewId === props.viewCd && row.username === props.username;
          })[0];
        }
        //console.log("saveGrpCd", saveGrpCd);

        if (saveGrpCd !== undefined && saveGrpCd.grpCd !== undefined) {
          baseGrpCd = saveGrpCd.grpCd;
        } else {
          baseGrpCd = props.userGrp;
        }
        //console.log("props.userGrp", props.userGrp, "baseGrpCd", baseGrpCd);

        const grpId = getUserGrpID(baseGrpCd);
        ...
        if (opts.length > 0) {
          ...
          setValue("userGrp", userGroupOptions[0].value);
        }
      } else {
        setValue("userGrp", userGroupOptions[0].value);
      }
    ...
    zAxios({
      ...
      .then(function (res) {
        ...
        res.data.map((item, idx) => {
          options.push({ label: transLangKey(item.name), value: item.id, gridCd: item.code });
        });
    ...
    zAxios({
      ...
      .then(function (res) {
        ...
        if (res.data) {
          ...
          res.data.map((entry, idx) => {
            options.push({ label: transLangKey(entry.grpNm), value: entry.id });
          });
        }
  ...
  const onGridGroupComboboxChanged = function (viewId) {
    ...
    showMessage(transLangKey("WARNING"), transLangKey("MSG_5048"), function (answer) {
      if (answer) {
        ...
        zAxios({
          ...
          .then((res) => {
            loadActual();
            reloadPrefInfoCallback(getUserGrpCd(getValues("userGrp")));
          })
          .catch((err) => {
            console.log(err);
          })
  ...
  return (
    <PopupDialog type="NOBUTTONS" open={props.open} onClose={props.onClose} onSubmit={handleSubmit(saveSubmit, onError)} title="PSNZ" resizeHeight={600} resizeWidth={840}>
      <SearchArea displaySize="small" expandButton={false} searchBtn={true}>
        <InputField name="gridNm" type="select" label={transLangKey("GRID")} readonly={false} disabled={false} control={control} options={codeNameMap} displaySize="small" />
        <InputField type="select" name="userGrp" label={transLangKey("USER_GRP")} control={control} options={userGroupOptions} displaySize="small" />
      </SearchArea>
      <Box sx={{ display: "flex", height: "100%", flexDirection: "column", alignContent: "stretch", alignItems: "stretch" }}>
        <ButtonArea title={"COLUMN_PSNZ"}>
          <RightButtonArea>
            <IconButton
              title={transLangKey("SAVE")}
              onClick={() => {
                saveData(psnzGrid);
              }}>
              <Icon.Save size={20} />
