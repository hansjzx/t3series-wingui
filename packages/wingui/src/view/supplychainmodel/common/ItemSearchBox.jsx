...
const defOptions = {
  itemCode: {
    ...
    avatarColor : 'primary.main', // display on PopOver
    usePopupButton: true
  },
  itemName: {
    code : 'NM',
...
export function ItemSearchBox(props, ref) {
      ...
      <>
        <Box className={classes.div} style={{display: style.display ? style.display : "block"}}>
          <FormControl className={`${classes.root} ${classes.searchBox}`} style={{ width: style.width}} variant="filled" size={"small"} >
            ...
            <FilledInput id="item" type="text" readOnly={inputReadOnly} ref={buttonRef} disableUnderline={true} value={inputValues} onChange={(event) => handleInputChange(event)} onKeyDown={(event) => handleKeyDown(event)}
        ...
        <Popover PaperProps={{sx:{boxShadow:'rgb(0 0 0 / 20%) 0px 1px 3px 0px, rgb(0 0 0 / 14%) 0px 1px 1px 0px, rgb(0 0 0 / 12%) 0px 2px 1px -1px'}}}
          ...
          <Box sx={popOverstyles} style={{height: style.popoverHeight ? style.popoverHeight : 250 , width: style.width}}>
                  ...
                  if (props.fields && props.fields.indexOf(key) == -1) {
                    ...
                    return (
                      <Grid key={key} className={hasFields && !(fields.includes("code") || fields.includes("cd")) ? "d-none" : "d-flex"} container direction="row">
                        <Grid item justifyContent="flex-start" alignItems="flex-start" sx={{ width: "115px" }}>
                        ...
                        <Grid item justifyContent="flex-start" alignItems="flex-start" sx={{ width: 'calc(100% - 115px)' }}>
                          <Controller name={key} control={control} render={({
                              ...
                              <TextField id={key} control={control} sx={{ width: '95%', margin : 0 }} variant="standard" onChange={onChange} value={value || ''}
                                ...
                                endAdornment: (
                                <InputAdornment position="end">
                                  { data.usePopupButton ? <CommonButton title={transLangKey("SEARCH")} onClick={openItemPopup}><Icon.Search /></CommonButton> : null }
  ...
  return (
    <>
      ...
      {itemPopupOpen && (<PopCommItem open={itemPopupOpen} onClose={closeItemPopup} confirm={onSetItem} url={props.url}></PopCommItem>)}
    </>
  )
}
