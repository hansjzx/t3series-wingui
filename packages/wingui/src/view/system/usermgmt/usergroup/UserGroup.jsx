...
import {
  ContentInner, SearchArea, ResultArea, ButtonArea, LeftButtonArea, RightButtonArea, SearchRow, zAxios,
...
function UserGroup() {
  ...
  function setPOptions() {
    ...
    userGrid.gridView.onCellClicked = function (grid, clickData) {
      ...
      if (clickData.column === 'displayName') {
        history.push({ pathname: '/setting/userrole/users', state: { username: clickedRow.username } })
      }
    }
  }
  function loadData() {
  ...
  function loadUserGrid(groupCode) {
    ...
    zAxios({
      method: 'get',
      url: baseURI() + 'system/groups/' + groupCode + '/users',
      waitOn: false
    }).then(function (res) {
