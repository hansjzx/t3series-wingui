...
function GroupPermission() {
  ...
  useEffect(() => {
    if (groupGrid !== null && viewPermissionGrid !== null && (grpCd !== null || gridCd !== '')) {
      groupGridLoadData()
  ...
  function setOptions() {
    groupGrid.gridView.onCellClicked = function (grid, clickData) {
      ...
      if (clickData.column === 'grpNm') {
        history.push({ pathname: '/setting/userrole/usergroup', state: { grpNm: clickedRow.grpNm } })
      }
    }
  }
  ...
  function groupGridLoadData() {
    ...
    zAxios.get(baseURI() + 'system/groups', {
      params: {
  ...
  function viewPermissionGridLoadData(grpCd) {
    ...
    zAxios.get(baseURI() + 'system/groups/' + grpCd + '/permissions', {waitOn: false})
      .then(function (res) {
