...
import {
  ...
  InputField, BaseGrid, GridSaveButton, useIconStyles, useViewStore, zAxios
...
function UserPermission() {
  ...
  useEffect(() => {
    if (location.state !== undefined && location.state !== null && pmsnGrid !== null && userGrid !== null) {
  ...
  }, [location, userGrid, pmsnGrid])
  useEffect(() => {
    if (userGrid !== null && pmsnGrid !== null) {
      userGridLoadData()
  ...
  function userGridLoadData() {
    ...
    zAxios.get(baseURI() + 'system/users', {
      params: {
  ...
  function pmsnGridLoadData(username) {
    ...
    zAxios.get(baseURI() + 'system/users/' + username + '/permissions', {waitOn: false})
      .then(function (res) {
    ...
    <ContentInner>
      ...
      <ResultArea sizes={[30, 63]} direction={"vertical"}>
        ...
        <Box className={classes.resultAreaBox}>
          ...
          <BaseGrid id="pmsnGrid" items={pmsnGridItems}></BaseGrid>
        </Box>
      </ResultArea>
    </ContentInner>
  );
