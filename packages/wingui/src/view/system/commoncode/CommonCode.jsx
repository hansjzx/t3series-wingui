...
import {
  ...
  InputField, BaseGrid, GridAddRowButton, GridDeleteRowButton, GridSaveButton, 
...
function CommonCode() {
  ...
  function loadData(targetGrid, srcId) {
    ...
    zAxios.get(baseURI() + 'system/common/' + seperator.url, {
  ...
  return (
    <ContentInner>
      ...
      <ResultArea style={{ height: "calc(100% - 53px" }}>
        <BaseGrid id="codeGrpGrid" items={codeGrpGridItems}></BaseGrid>
      </ResultArea>
      <ButtonArea>
        <LeftButtonArea>
      ...
      <ResultArea style={{ height: "calc(40% - 22px)" }}>
        <BaseGrid id="codeGrid" items={codeGridItems}></BaseGrid>
      </ResultArea>
    </ContentInner>
  );
}
