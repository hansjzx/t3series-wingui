...
import { ButtonArea, LeftButtonArea, RightButtonArea, BaseGrid, GridAddRowButton, GridDeleteRowButton, GridSaveButton, useViewStore, zAxios, PopupDialog } from "@zionex/wingui-core/src/common/imports";
...
function PopPreferenceOptions(props) {
  ...
  const onDelete = (targetGrid, deleteRows) => {
    if (deleteRows.length > 0) {
      return zAxios({
        method: 'post',
        url: baseURI() + 'system/users/preference-options/delete',
        headers: { 'content-type': 'application/json' },
        data: deleteRows
      })
    }
