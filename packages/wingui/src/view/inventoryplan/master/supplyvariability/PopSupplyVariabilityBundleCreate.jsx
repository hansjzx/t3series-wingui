...
import { InputField, useUserStore, zAxios, PopupDialog } from '@zionex/wingui-core/src/common/imports';

function PopSupplyVariabilityBundleCreate(props) {
  ...
  function saveSubmit() {
    ...
    param.append('OVERWRITE_YN', getValues('OVERWRITE_DATA_YN').includes('Y'));
    ...
    zAxios({
      ...
      url: baseURI() + 'engine/mp/SRV_UI_IM_10_BATCH',
      params: param,
      fromPopup: true
    })
    .then(function (res) {
      if (res.status === gHttpStatus.SUCCESS) {
        if (res.data.RESULT_SUCCESS) {
          showMessage(transLangKey('MSG_CONFIRM'), transLangKey(res.data.RESULT_DATA.IM_DATA.SP_UI_IM_10_BATCH_P_RT_MSG), { close: false });
        } else {
  ...
  return (
    <>
    <PopupDialog open={props.open} onClose={props.onClose} onSubmit={handleSubmit(saveSubmit, onError)} title='POP_UI_IM_10_02' resizeHeight={230} resizeWidth={400}>
...
export default PopSupplyVariabilityBundleCreate;
